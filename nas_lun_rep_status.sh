...
LUNNAME=$($API -s --exec api=SYNO.Core.ISCSI.LUN version=1 method=list | jq -r '.data.luns[] | select(.uuid=="'"$LUNUUID"'")' | jq -r .name)
...
then
  ...
  then
    mapfile -t RESULT < <(jq .recent_records[-1] < /volume1/@appconf/SnapshotReplication/plan/"$PLAN"/sync_report | jq -r .begin_time,.finish_time,.sync_size_byte)
